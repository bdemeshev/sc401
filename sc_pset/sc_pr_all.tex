...
\begin{sol}
Нет. В конченой только $2^n$, где $n$ — количество элементов в разбиении $\Omega$
\end{sol}
\end{problem}
...
\begin{problem}
Пусть $(\Omega,\mathcal{F},P)$ — вероятносное пространство, $X$ -
...
\begin{enumerate}
\item Какой вероятностный смысл имеет величины $f'(1)$,
$f''(1)+f'(1)-(f'(1))^{2}$?
...
б) Верно ли, что игра зкончится к моменту $n_{x}+n_{y}$? \\
...
Пусть $t_{1}<t_{2}<\ldots<t_{n}$ — фиксированне моменты времени. \\
...
\begin{sol}
Способ раз. Продифференцировать подинтегральное выражение. Способ два. Записать этот интеграл в краткой записи и получить обычное дифференциальное уравнение.
\end{sol}
\end{problem}
