\section{1. Счетность множеств и пр.}

\problem{ % in sc_skelet
Какова мощность декартово произведение счетного количества счетных множеств? }
\solution{Континуум, т.к. оно равномощно множеству последовательностей из натуральных чисел.}

\problem{Пусть $A$ - некоторое множество. Пусть $F$ - множество функций принимающих только значения ноль или один с областью определения $A$. Пусть $M$ - множество всех подмножеств $A$. Равномощны ли $F$ и $M$?}
\solution{да}

\problem{ An enemy submarine is somewhere on the number line (consider only integers for this problem). It is moving at some rate (again, integral units per minute). You know neither its position nor its velocity.\\
...
\solution{ Idea of solution: \\
Счетное количество счетных множеств, считаем змейкой }
...
\solution{Одинаковая, биекция: $\{2,4,5\}\sim 01011000000...$, т.е. единички соответствуют включенным в подмножество числам}

\problem{Сравните мощности множеств $ A $ и $ B $, если $ A=\mathbb{Q} $ --- рациональные числа, $ B=\mathbb{Q}^{2} $ --- пары рациональных чисел.}
\solution{Одинаковая. $ \mathbb{Q}\sim \mathbb{N}\sim \mathbb{N}^{2}\sim \mathbb{Q}^{2} $}
...
4.3. $A+0=A$ \\
4.4. $A-0=A$ \\
а) Какие операции над множествами скрыты значками $+$, $-$,
$\cdot$? \\
b) Какие множества подразумеваются под $1$ и $0$? \\
с) Упростите в этой алгебре выражение $(A+B)(A-B)+A$ \\
...
\problem{Приведите пример алгебры, которая не является $\sigma$-алгеброй.}
\solution{Пример. $\Omega=[0;1]$, $\mathcal{A}$ состоит из конечных множеств и множеств с конечными дополнениями.}


\problem{
Сколько $\sigma$-алгебр можно создать на множестве из 4-х элементов? }
\solution{столько же, сколько разбиений }
...
\problem{
Пусть $A$ и $B$ - события. Определим $C_{n}=\left\{
\begin{array}{ll}
  A, & n=2k \\
  B, & n=2k+1 \\
\end{array} \right.$. \\
Найдите $\limsup_{n}C_{n}$ и $\liminf_{n}C_{n}$.}
\solution{$\limsup_{n}C_{n}=A\cup B$, $\liminf_{n}C_{n}=A\cap B$. }


\problem{
Пусть $A_{1} \subseteq A_{2} \subseteq A_{3} \subseteq ...$.
Найдите $\liminf_{n}A_{n}$ и $\limsup_{n}A_{n}$. \\
Пусть $B_{1} \supseteq B_{2} \supseteq B_{3} \supseteq ...$.
...
\solution{$\liminf_{n}A_{n} \subseteq \liminf_{n}B_{n}$ и $\limsup_{n}A_{n}
...
Найдите $1_{A}+1_{A^{c}}$ }
\solution{ $1_{A\cap B}=1_{A}1_{B}$, $1_{A\backslash B}=1_{A}1_{B^{c}}$, $1_{A}+1_{A^{c}}=1$}
...
\solution{ б) $\Omega=\{a,b,c\}$, $\mathcal{F}_{1}=\{\emptyset,\Omega,a,bc\}$, $\mathcal{F}_{2}=\{\emptyset,\Omega,ab,c\}$}
...
\problem{Продолжение про тяжелую долю чисел. \\
Верно ли, что у натуральных чисел, в записи которых присутствует
хотя бы одна единица, есть доля? 
Верно ли, что у натуральных чисел, в записи которых присутствует
ровно одна единица, есть доля? }
\solution{}
...
а) Опишите все множества, входящие в $\mathcal{A}$ \\
б) Опишите все множества, входящие в $\mathcal{F}$ \\
в) Верно ли, что $\mathcal{A}$ и $\mathcal{F}$ совпадают? }
\solution{}
...
\problem{ Правильный кубик подбрасывается один раз. $Y$ - индикатор того, выпала ли четная грань. $Z$ - индикатор того, выпало ли число больше 2-х. \\
\begin{itemize}
...
\solution{Например, $B$ - Канторово множество, или (гораздо проще)
...
\problem{
We toss a fair coin infinitly many times. So the set of all oucomes $\Omega$ is the set of all infinite sequences of H and T. Let $A_{n}$ be the event that the coin shows head in the $n$-th toss. Let $\mathcal{F}=\sigma(A_{1},A_{2},A_{3},...)$ - be the minimal $\sigma$-algebra containing all the $A_{n}$. \\
...
\problem{Пусть $f:\mathbb{R}\to\mathbb{R}$ - дифференциируемая функция. Верно ли, что $f'$ - измерима по Борелю?}
\solution{Да. $ f' $ - непрерывна, а следовательно, измерима.}

\problem{Пусть $\Omega=[0;1]$, $ \mathcal{F}=\mathcal{B}[0;1] $. Случайный процесс $X_{t}(w):=1_{t=w}$. Постройте фильтрацию, порождаемую этим процессом.}
\solution{Заметим, что $\sigma(X_{t})=\{\emptyset,\Omega,\{t\},\Omega\backslash\{t\}\}$. Значит в $ \mathcal{F}_{t} $ является минимальной $\sigma  $-алгеброй порожденной набором одноточечных множеств $\{x|x\in [0;t]\}$. Т.е. $\mathcal{F}_{t}$ - это все счетные подмножества отрезка $[0;t]$ плюс все их дополнения.}



\problem{В лесу есть три вида грибов: рыжики, лисички и мухоморы. Попадаются они равновероятно и независимо друг от друга. Маша нашла три гриба. Пусть $R$ --- количество рыжиков, $ L $ --- количество лисичек, а $ M $ --- количество мухоморов среди этих трех грибов.
\begin{itemize}
...
\end{itemize} }
\solution{$ 2^{4} $, $2^{10}$, нет, да }


\problem{Сначала подбрасываем кубик. Если выпала нечетная грань, то подбрасываем правильную монетку один раз. Если четная --- то два. Пусть $X$ - количество подбрасываний монетки, а $Y$ - количество выпавших орлов.
...
\solution{ $ 2^{2} $, $ 2^{3} $, $ 2^{5} $, нет, нет}

\problem{Пусть $X_{n}$ - независимые случайные величины равновероятно равные плюс или минус единице. $S_{n}=X_{1}+X_{2}+...+X_{n}$. Сравните $ \sigma $-алгебры $ \sigma(X_{1},X_{2},...X_{n}) $ и $ \sigma(S_{1},S_{2},...S_{n}) $.}
\solution{Они совпадают}
...
\problem{Let $ S_{n} $ be a symmetric random walk, i.e. $ S_{n}=X_{1}+X_{2}+...+X_{n} $ where $ X_{n} $ are independent and identically distributed with $ \P(X_{n}=1)=\P(X_{n}=-1)=0.5 $. Let $T$ be the time of the second local maximum plus one. For example: if the sequence of $ X_{n} $ is given by $ +1,+1,-1,-1,-1,+1,-1,-1,+1,... $ then $ T= 7$. We define the following sigma-algebras: $ \mathcal{F}_{n}:=\sigma(X_{1},X_{2},...,X_{n})$ and $ \mathcal{F}_{T} $ - all events which may be distninguished by a rational agent who knows all $ X_{i} $ up to time $ T $. 
\begin{itemize}
...
\problem{Опишите $\sigma$-алгебру подмножеств отрезка $[0;1]$ порожденную отрезками $[0;0.1)$ и $(0.5,1]$.}
\solution{Содержит 8 элементов, порождается разбиением.}

\problem{Случайная величина $X$ равномерна на отрезке $[0;1]$. Величина $Y$ определяется по формуле $Y=1_{X<0.5}+1_{X>0.3}$. Опишите $\sigma$-алгебру $\sigma(Y)$.}
\solution{Содержит 4 элемента, порождается разбиением.}


\section{3. Модель в дискретном времени, дерево}

\problem{ Consider a stock whose price today is \$50. Suppose that over the next
...
\problem{ A stock price is currently \$50.  It is known that at the end of 6
...
$n$  and the outcome of the coin tossing $\om_1\dots\om_{n+1},$ then $\fr{S_n}{(1-a)^n(1+r)^n}$ is
...
expiring at time $m$ and having strike price $\fr{K}{(1+r)^{N-m}}.$}
\solution{}

\problem{Asian option. Consider an $N$-period binomial model.
An \emph{Asian option} has a payoff based on the average stock price, i.e.,
$$
V_N=f\bl(\fr{1}{N+1}\sum\lims_{n=0}^{N} S_n \br),
$$
...
\problem{ Asian option continued) Consider an $N$-period binomial model,
and let $M$ be a fixed number between $0$ and $N-1$. Consider an Asian option with the payoff
at time $N$ defined as
$$
V_N=f\bl(\fr{1}{N_M}\sum\lims_{n=M+1}^{N} S_n \br),
$$
...
\problem{ Докажите, что адаптированные случайные процессы $\bl\{ \fr{S_n}{(1+r)^n}\br\}$ и
  $\bl\{ \fr{X_n}{(1+r)^n}\br\}$ являются мартингалами относительно риск-нейтральной 
  меры $(\ti p, \ti q)$. Условие $0<d<1+r<u$ является естественным \emph{необходимым} условием 
  отсутствия арбитража. Доказав что $\bl\{ \fr{X_n}{(1+r)^n}\br\}$ мартингал, вы покажете, 
...
\problem{ Имеется три опциона колл с одинаковым сроком исполнения и разными страйк-ценами: $K_{1}<K_{2}<K_{3}$, причем $K_{2}-K_{1}=K_{3}-K_{2}$. Обозначим цены этих опционов как $C_{1}$, $C_{2}$ и $C_{3}$. Сравните $C_{2}$ и $\frac{C_{1}+C_{3}}{2}$  }
\solution{}


\problem{ Пусть $S_{T}$ - цена акции в момент $T$. Экзотический опцион выплачивает сумму $g(S_{T})=min\{max\{S_{T},100\},120\}$. Выразите цену этого опциона через цены двух подходящих опционов колл и процентную ставку. }
\solution{}
...
\problem{Известно, что $ \P(X>c \Delta Y>c)=0 $ для любой константы $ c $. Верно ли, что $X=Y$ почти наверное?}

\solution{Да. Рассмотрим множество $ X-Y>c $. Его можно представить в виде $ \{X-Y>c\}=\cup_{q} \{ X>q \cap Y<q-c\} $. Далее замечаем, что $ \P(X>q \cap Y<q-c)=\P(Y>q \cap Y<q-c)=0 $ для $ c>0 $. Т.е. $ \P(X-Y>c)=0 $ для $ c>0 $. В силу непрерывности вероятности получаем $ \P(X-Y>0)=0 $. Далее используем симметрию и получаем $ \P(Y-X>0)=0 $. Значит $ \P(X=Y)=1 $}

\problem{Имеется последовательность событий $A_1$, $A_2$, \ldots, такая что вероятность любого $A_i$ равна $\P(A_i)=1$. Найдите $\P(\cup A_i)$ и $\P(\cap A_i)$}
\solution{Обе равны 1}

\problem{Имеется последовательность событий $A_1$, $A_2$, \ldots, такая что $\lim \P(A_i)=1$. Найдите $\lim \P(B\cap A_i)$}
\solution{$\P(B)$}

\problem{Существует ли последовательность событий $A_1$, $A_2$, \ldots, такая что $\lim \P(A_i)=1$, но $\P(\cap_{i=k}^{\infty} A_i)=0$ для любого числа $k$?}
\solution{Да}


\section{5. Виды сходимостей}
...
\problem{
$[$Williams, E4.7$]$ \\
Ваш выигрыш в $n$-ой партии равен $X_{n}=\left\{
\begin{array}{ll}
  n^{2}-1, & p=n^{-2} \\
  -1, & p=1-n^{-2} \\
\end{array}\right.$ и $X_{n}$ - независимы. \\
а) Найдите $\E(X_{n})$ \\
...
сходится? \\
в) К чему и в каком смысле сходится $Z_{n}=(U_{1}\cdot...\cdot U_{n})^{1/n}$? }
\solution{}
...
\problem{
Пусть величины $X_{1}$, $X_{2}$,... имеют нулевое среднее, ограниченную общей константой дисперсию и некоррелированы. Пусть $S_{n}=X_{1}+...+X_{n}$ и $\alpha>1/2$.\\
Сходится ли $\frac{S_{n}}{n^{\alpha}}$ в $L^{2}$? }
\solution{}


\problem{ breaking a stick \\
...
\problem{Let $X_{n}=X\cdot 1_{X\leq n}$. In what sense (as., in $L^{2}$, in probability, in law) does the sequence $ \{X_{n}\} $ converge? What is the limit? Consider two cases:
\begin{itemize}
...
\solution{ $ X_{n}\to X $. a) as, P, law, $ L^{2} $; b) as, P, law }


\problem{Случайная величина $X_1$ равномерна на отрезке $[0;1]$ и $X_n=X_1^n$. В каких смыслах и к чему сходится $X_n$?}
\solution{К нулю, кроме как поточечно.}

\problem{Последовательность случайных величин $X_n$ такова, что $\P(X_n\to 0)=1$. Возможно ли, что $\lim \E(X_n)\neq 0$?}
\solution{}


\problem{Известно, что $X_n \xrightarrow{\P} 0$. Верно ли, что $\Var(X_n)\to 0$? }
\solution{Нет.}

\problem{Известно, что $X_n \xrightarrow{\P} X$. Верно ли, что $Y\cdot X_n \xrightarrow{\P} YX$? }
\solution{Да}


\problem{Известно, что $X_n \xrightarrow{\P} X$ и $Y_n \xrightarrow{\P} Y$. Верно ли, что $X_nY_n \xrightarrow{\P} XY$? }
\solution{Да}


\problem{Известно, что $\varepsilon>0$. Сравните величины $\P(X\leq t)$ и $\P(Y\leq t+\varepsilon)+\P(|X-Y|>\varepsilon)$.}
\solution{$\P(X\leq t)$ меньше}

\problem{Какое условие является более сильным?
\begin{enumerate}
...
\]
\end{enumerate}}
\solution{Условия равносильны}

\problem{Последовательность случайных величин $X_n$ поточечно сходится к случайной величине $X$. Кроме того, $\lim \E(X_n)=0$. 
\begin{enumerate}
...
\problem{Последовательность случайных величин $X_n$ сходится к $a$ по вероятности. Функция $f$ --- борелевская и имеет производную в точке $x=a$. Представим $f(X_n)$ в виде
\begin{equation}
f(X_n)=f(a)+f'(a)(X_n-a)+Y_n(X_n-a)
\end{equation}
Сходится ли $Y_n$ по вероятности? Если да, то к чему?}
\solution{Сходится по вероятности к нулю}

\problem{Известно, что $X_n$ сходится по вероятности к нулю и $\lim_{t\to\infty} \sup_n \P(|Y_n|>t)=0$. Верно ли, что $X_n Y_n$ сходится по вероятности к нулю?}
\solution{да,
\begin{multline}
...
\problem{Хвостовая $\sigma$-алгебра. Случайные величины $X_1$, $X_2$, \ldots независимы, $Y_n=X_1\cdot X_2\cdot\ldots\cdot X_n$. Каждая $X_i$ равновероятно принимает значения $0.5$ и $1.5$. 
\begin{enumerate}
...
\end{enumerate}}
\solution{Событие $Y_n\to 0$ лежит в хвостовой $\sigma$-алгебре, поэтому $\P(Y_n\to 0)$ может равняться только нулю или единице.}

\section{6. Интеграл Лебега}
...
значений $\{a_{1},a_{2},...\}$, где $a_{i} \ge 0$. Докажите, что
...
\problem{Известно, что $\Var(X)=0$. Существует ли константа $a$ такая, что $0<\P(X=a)<1$?}
\solution{Нет, $X$ --- почти наверное константа.}

\problem{Известно, что $\P(0<X<1)=1$. Сравните $\Var(X)$ и $\E(X)$.}
\solution{$\Var(X)<\E(X)$}

\problem{Задана последовательность функций $f_n(x)=\frac{n^2 x e^{-n^2 x^2}}{1+x^2}$. Сходится ли числовая последовательность $a_n=\int_{[0;+\infty)}f_n d\lambda$, где $\lambda$ --- классическая мера Лебега?}
\solution{}


\problem{Функции $f_n$ заданы на отрезке $[0;1]$ и интегрируемы. Последовательность $f_n$ сходится равномерно к функции $f$ по классической мере Лебега $\lambda$. Верно ли, что $\int_{[0;1]} f\,d\lambda$ существует? Верно ли, что $\int_{[0;1]} f_n\,d\lambda\to \int_{[0;1]} f\,d\lambda$?}
\solution{Да}


\problem{Функции $f_n$ заданы на множестве $[0;\infty)$ формулой $f_n(x)=2nxe^{-nx^2}$. Существует ли  интегрируемая функция $f$, такая что $f\geq f_n$ для всех $n$?}
\solution{}


\section{7. Характеристическая функция}
...
б) Докажите, что $\E(X^{2n})=1\cdot 3\cdot 5\cdot...\cdot(2n-1)$ \\
...
\problem{  Случайной величине $X$, чей закон распределения задан таблицей: \\
...
\solution{ берем любые две характеристические функции, чтобы получалось в итоге нормальное распределение. }
...
\problem{Вася хватается за точку $t$ на отрезке $[0;1]$. Затем на отрезке делается $n$ равномерно выбираемых случайных разрезов.
...
Затем замечаем, что $\frac{1}{L+1}=\int_{0}^{1}x^{L}$. Значит $\E(\frac{1}{L+1})=\int_{0}^{1}\E(x^{L})=...=\frac{1-(1-t)^{n+1}}{t(n+1)}$.
...
\section{8. Условное ожидание}

\problem{ Borel-Kolmogorov paradox \\
...
а) $X$ - эскпоненциально распределено с $\lambda$ \\
...
\problem{ Пусть $X$ и $Y$ - интегрируемые случайные величины, такие что
$\E(X|Y)=Y$ и $\E(Y|X)=X$. Верно ли, что $X=Y$ (as)? }
\solution{Да, для $X$ и $Y$ из $L^2$ доказательство простое: $\E(X^2)=\E(Y^2)=\E(XY)$ и $\E((X-Y)^2)=0$. В общем случае нужно спускаться до событий и определения (?). 
...
Найдите $\E(X_{1}|\min\{X_{1},...,X_{n}\})$ и
$\E(X_{1}|\max\{X_{1},...,X_{n}\})$ }
\solution{
$p(x_{1}|\min)=\frac{n-1}{n(1-\min)}$ при $x_{1}>\min$ \\
$\E(X_{1}|\min\{X_{1},...,X_{n}\})=\frac{(1+\min)(n-1)}{2n}$ }
...
\problem{
Let X1...Xn be indepdently and identically distribuetd random variables, strictly positive. \\
Show that $\E(S_{m}|S_{n})= m/n$ for $m\le n$ , where $S_{m} = X_{1} + X_{2} + ... + X_{m}$ }
\solution{}
...
\problem{ Let $X$ be an exponential random variable, so that $\P[X > t] = e^{-\lambda t}$ 
...
\problem{
Пусть клиенты приходят пуассоновским потоком с интенсивностью $\lambda_{in}$. А время обслуживания одного клиента эскпоненциально с параметром $\lambda_{service}$. \\
Пусть $X$ - число клиентов, пришедших за время обслуживания Васи Сидорова. \\
Как распределено $X$? }
\solution{
$\P(X=k)=\E(1_{X=k})=\E(\E(1_{X=k}|T))=\int_{0}^{\infty}e^{-lambda_{in}t}\frac{(\lambda_{in}t)^{k}}{k!}\lambda_{service}e^{-\lambda_{service}t}dt=...$ \\
...
\solution{ $p(y)=\frac{\lambda}{(\lambda+y)^{2}}$, $\E(Y|X)=1/X$, $\E(X|Y)=\frac{2}{\lambda+Y}$ }
...
\solution{ первое и четвертое верны, а второе и третье --- нет.}
...
\problem{ Пусть $X$ и $Y$ - независимые случайные величины, $X$ - $Bin(n_{1},p)$, $Y$ - $Bin(n_{2},p)$. Найдите $\E(X|X+Y)$.}
...
\problem{Вася, Петя и Коля играют в карточного <<дурака>> втроём. Вася проигрывает с вероятностью $p_1$, Петя --- с вероятностью $p_2$, Коля --- с вероятностью $p_3$. Естественно, $p_1+p_2+p_3=1$. Всего они сыграли $n$ партий. Обозначим количества проигранных ими партий $X_1$, $X_2$ и $X_3$, соответственно. Найдите $\E(X_1|X_1+X_2)$. Может получится и $\Var(X_1|X_1+X_2)$?}
\solution{Раскладываем $X_1$ в сумму $n$ индикаторов, получаем $\E(X_1|X_1+X_2)=\ldots=\frac{p_1}{p_1+p_2}(X_1+X_2)$}


\problem{Пусть $X_{1}\sim U[0;1]$, $X_{2}\sim U[-1;1]$, $X_{3}\sim U[-1;2]$. И $Y_{i}=X_{i}^{2}$. Найдите: а) $ \E(X_{1}|Y_{1}) $, $ \E(X_{2}|Y_{2}) $ и $ \E(X_{3}|Y_{3}) $.}
\solution{ $Y_{1}$, $0$, $Y_{3}1_{Y_{3}>1}$ }

\problem{
Let $ X $ be uniform on $[0;1]$. We define $ A:=\{X>0.1\} $ and $ Y:=X^{2} $. Find $ \E(Y|\sigma(1_{A})) $, $\E(1_{A}|\sigma(Y))$ and $\E(1_{A}+Y|Y-1_{A})$.}
\solution{}

\problem{Время между появлением автобусов имеет экспоненциальное распределение с $ \lambda=1 $ (функция плотности $ p(t)=exp(-t) $ при $ t>0 $). Вася приходит на остановку и ждет до первого автобуса. Допустим он ждал $ T $ минут. На следующий день он ждет автобуса в течение времени $ T $. Какова вероятность того, что за это время придет автобус?}
\solution{$ 1/2 $}


\problem{Пусть $ X_{1} $, ..., $ X_{100} $ независимы и равномерны на $ [0;1] $. Пусть $ L=\max\{X_{1},X_{2},\ldots, X_{80}\} $ а $ R=\max\{X_{81},X_{82},\ldots,X_{100}\} $ и $ M=\max\{X_{1},\ldots,X_{100}\} $ Найдите $ \P(L>R|L)$ и $ \P(L>R|R) $ и $ \P(L>R|M)$, $\P(L>R|L,M) $}
\solution{$ \P(L>R|L)=L^{20}$,  $ \P(L>R|R)=1-R^{80} $, $ \P(L>R|M)=\frac{80}{100}$, $\P(L>R|L,M)=1-1_{L>M} $  }

\problem{Пусть $ X_{1} $, ..., $ X_{n} $ - независимы и равномерны на $ [0;1] $, и $ M=max\{X_{1},\ldots,X_{n}\} $. Найдите $\E(M^{k})$ в уме без вычислений. Подсказка: $ M=\P(M>X_{n+1}|M) $
}
\solution{Используя подсказку получаем: $ M^{k}=\P(M>X_{n+1} \cap M>X_{n+2} \cap \ldots \cap M>X_{n+k}|M) $. Значит $ \E(M^{k})=\P($максимум из $ (n+k) $ равномерных величин лежит среди $ n $ первых$)=\frac{n}{n+k} $}

\problem{Известно, что $ Y $ и $ X=\E(Y|\mathcal{H}) $ одинаково распределены. Верно ли, что $ Y=X $ почти наверное? Hint: start by comparing sets $ Y>0 $ and $ X>0 $}
\solution{Во-первых, интуитивно: наше условное мат. ожидание не сделало $ Y $ более <<грубым>>, значит не должно было совсем изменить, т.е. ответ <<верно>>. Во-вторых, формально. Т.к. $ X $ и $ Y $ одинаково распределены:
...
\problem{Верно ли, что $\E(X|Y^{2})=\E(X|\,|Y|)$?}
\solution{Да, т.к. $\sigma(Y^{2})=\sigma(|Y|)$}

\problem{Величины $X_1$, $X_2$, ... независимы и одинаково распределены. Мы складываем случайное количество $N$ слагаемых. Обозначим сумму буквой $S=\sum_{i=1}^{N}X_i$. Найдите $\E(S)$, $\Var(S)$ и $Cov(S,N)$}
\solution{ $Cov(S,N)=\E(SN)-\E(S)\E(N)=\E(\E(SN|N))-\E(\E(S|N))\E(N)=...$ }


\problem{Неправильный кубик выпадает с вероятностью $0{,}5$ шестеркой вверх. Остальные пять граней выпадают равновероятно. Случайная величина $X$ -- остаток от деления номера грани на два, $Y$ -- остаток от деления номера грани на три. Найдите
\begin{enumerate}
...
\problem{Случайная величина $X$ принимает значения в диапазоне $[0;1]$. Всегда ли существует случайная величина $Y$ принимающая всего два значения, такая что $E(Y|X)=X$? Такая что $E(X|Y)=Y$? }
\solution{а) рассмотрите случай $Y$ принимающей два значения, 0 и 1 б)}


\problem{Цена литра молока, $X$, распределена равномерно на отрезке $[1;2]$. Количество молока, которое дает корова Мурка, $Y$, распределено экспоненциально с $\lambda=1$. Надои не зависят от цены. Величина $Z$ --- выручка кота Матроскина от продажи всего объема молока.
...
\end{enumerate}}
\solution{ $E(X)=1$, $\Var(X)=1$, $E(X^2)=\Var(X)+E^2(X)=2$; 
...
\problem{Кубик подбрасывают бесконечное количество раз. Величина $X$ --- номер подбрасывания, когда впервые выпала единица, а $Y$ --- номер подбрасывания, когда впервые выпала шестерка. Найдите $\E(Y|X)$.}
\solution{}

\problem{Верно ли что $\E(XY)=\E(\E(X\mid Z)\cdot \E(Y\mid X))$, если левая и правая части определены?}
\solution{Нет, например, возьмем независимые одинаково распределенные $X$ и $Y$ и $Z=(X+Y)/2$.}

\problem{The random variables $X_1$, $X_2$, \ldots are independent uniformly distributed on $[0;1]$. I~am summing them until the first $X_i$ greater than $0.5$ is added. After this term I stop. Let's denote by $S$ the total sum and by $N$ --- the number of terms added. Find $\E(S|N)$, $\Var(S|N)$, $\E(S)$, $\Var(S)$}
\solution{}


\section{9. Разложение в сумму, первый шаг, цепи Маркова}
...
\problem{
Пусть $S_{n}$ - случайное блуждание, $S_{n}-S_{n-1}=\left\{
\begin{array}{rl}
  1, & p \\
  -1, & 1-p \\
\end{array}
...
\problem{ The following example is a classic in elementary probability theory called
...
\problem{   Consider a coin-flipping game, where the probability of a head is
...
%%%%%
\problem{ In a random walk starting at the origin find the probability
...
%%%%%
\problem{ Show that in a random walk starting at the origin the
...
\problem{
$[$Steele, 2.1.$]$ \\
$S_{n}=S_{n-1}+X_{n}$, $X_{n}= \left\{ \begin{array}{rcl} -1,
p=0,52 \\ 1, p=0,45 \\ 2, p=0,03 \\ \end{array} \right.$ \\
...
информацией $\mathcal{F}_{n}=\sigma(X_{1},...,X_{n})$. На $n+1$-ую
...
\problem{Известно, что $X_{n}$ - субмартингал по отношению к фильтрации $\mathcal{F}_{n}$. Также известно, что $\E(X_{n})=const$. Верно ли, что $X_{n}$ - мартингал?}
\solution{да}
...
\problem{ Show that a convex function of a martingale is a submartingale.
...
\problem{Problem 1.3 (Discrete time stochastic integral) Suppose $M_0,M_1,\dots,M_N$
...
\problem{ Рассмотрим серию испытаний Бернулли с подбрасыванием монеты. Положим $X_j=1,$ 
  ...
  $$
  \ni Докажите, что $M_0,M_1,\dots$ матингал. Пусть $\sgm>0.$ 
  Положим $S_n=e^{\sgm M_n}\cdot\bl(\fr{2}{e^{\sgm}+e^{-\sgm}} \br)^n$
  \ni Докажите, что процесс $S_0,S_1,S_2,\dots$ также мартингал.}
\solution{}

\problem{ Дискретный стохастический интеграл. Пусть процесс $M_0,M_1,\dots,M_N$ является
...
\problem{ Toss a symmetric coin repeatedly. Let $X_j=1$ if the $j$-th toss
...
\item[(ii)] Let $\sgm$ be a positive constant and, for $n\ge 0$, define
$$
S_n=e^{\sgm M_n}\cdot\bl(\fr{2}{e^{\sgm}+e^{-\sgm}} \br)^n
$$
...
$e^{\sgm M_n}$ does have a tendency to grow. This is the result of putting a martingale into
the convex exponential function. In order to again have a martingale, we must "discount" the
geometric symmetric random walk, using the term  $\fr{2}{e^{\sgm}+e^{-\sgm}}$ as the discount rate.
\end{itemize}}
\solution{}
...
т.д., когда слово кончается все деньги снова ставятся на A...).
...
\problem{ Намек на сходимость мартингалов. \\
...
\problem{ Let $M_0,M_1,M_2\dots$ ne the symmetric random walk. Define $I_0=0$ and
$$
I_n=\sum\lims_{j=0}^{n-1} M_j(M_{j+1}-M_j),\quad n=1,2,\dots
$$
\ni Show that
$$
I_n=\fr{1}{2}M_n^2-\fr{n}{2}.
$$}
\solution{}
...
\problem{ Пусть $T$, $S$ - моменты остановки. \\
...
\problem{ Равномерная интегрируемость... Докажите (или опровергните)... \\
...
\solution{$ \left(\frac{2}{3}\right)^{B_{t}} $ - мартингал}
...
а) Какова вероятность того, что шары будут окрашены в красный цвет? \\
б) Сколько в среднем пар шаров будет извлечено? }
\solution{
Hint: $K_{t}$ - число красных шаров - мартингал, }
...
\problem{Величины $X_i$ независимы, одинаково распределены и строго положительны, $S_n=\sum_{i=1}^n X_i$. Число $x>0$. Момент остановки $T_x=\min\{n|S_n\geq x\}$. Докажите неравенство $x\leq \E(\min\{X_1,x\})\E(T_x)\leq 2x$}
\solution{По тождеству Вольда
\begin{equation}
...
Решение 3. Обозначим Васину координату в момент времени $t$ как $X_t$. Можно найти $a$ так чтобы процесс $Y_t=a^{X_t}$ был мартингалом. Момент остановки $T=\min\{t|X_t\geq n\}$. Мартингал $Y_{t\wedge T}$ ограничен, теорема Дуба применима. $\E(Y_T)=\E(Y_0)=1$. Получаем уравнение $P_n a^{n}+(1-P_n) a^{n+1}=1$.}
...
\solution{Ответ: к 0, пожалуй во всех кроме поточенчном }



%%%%%
\problem{  Let $Z$ be a normally distributed random variable, with mean
...
%%%%%
\problem{ Let $W_1(t)$ be a Brownian motion and $W_2(t)$ be another
...
%%%%%
\problem{ What is the distribution of $W(s) + W(t)$, for $0 \le s \le
...
%%%%%
\problem{  Show that the probability density function
  $$
...
%%%%%
\problem{ Let $ W(t) $ be standard Brownian motion.
  \begin{enumerate}
      \item
...
%%%%%
\problem{  Show that \( \E[T_a] = \infty \) for $ a > 0 $.}
\solution{}
...
%%%%%
\problem{ \tbf{The Distribution of the Maximum} \\
  Let $t$ be a given time, let $a>0$ be a given value, then
  \begin{eqnarray*}
    \P\{ \max_{0\le u \le t} W(u) \ge a \} &=& \P\{ T_a \le t\} \\
    &=& \fr{2}{\sqrt{2\pi}} \int_{a/\sqrt{t}}^\infty \exp(-y^2/2)\;dy
  \end{eqnarray*}}
\solution{}


%%%%%
\problem{ \tbf{Quadratic Variation.} \\
  ...
  $ a = t_0 < t _1 < ... < t_n < t_{n+1} = b$, with mesh size
  ...
  defined by $S_n = X_1 + ... + X_n$ with the Bernoulli random variables 
...
\solution{  Introduce some briefer notation for the proof, let:
  $$
    \Dt_{nk} = 
                  W\left( \frac{k}{2^n} t \right) - 
                  W\left( \frac{k-1}{2^n} t \right)
...
%%%%%
\problem{ {\it Comment}:  Starting from
  $$
    \lim_{n \to \infty}
             \sum_{n=1}^{2^n} 
               \left[ 
...
\problem{
  ...
  $$
    \P\bl\{ W(s) \in (x,x+\Delta x) |\, W(t) = B\br\} = 
    \fr{1}{\sqrt{2\pi (s/t)(t-s)}}\cd \exp(-(x-Bs/t)^2/2(t-s)) \cd\Dt x.
  $$
}
\solution{
...
%%%%%
\problem{ Условная плотность $W(t),$ где $t_1 < t < t_2,$ пр условии, что 
...
\solution{$ X(t) $ subject to the conditions $ X(t_1) = A $ and $ X(t_2)
...
%%%%%
\problem{ The above theorem can be amusingly summarized in the following
...
\problem{ \tbf{Hitting Times.} \\
  ...
  (p.d.f)
  $$
    f_{T_a}(t) = \fr{a}{\sqrt{2\pi}} t^{-3/2} \exp(-a^2/(2t)).
  $$
...
%%%%%
\problem{ \tbf{Hitting Times Continued.} \\
  ...
  $$
    \P\{\text{ to $a$ before $-b$ }\} = \fr{b \Dt x}{(a + b)\Dt x} = \fr{b}{a+b}.
  $$
...
%%%%%
\problem{ \tbf{VERY DIFFICULT.} Prove that with probability $1$ (i.e. almost surely) a 
  Brownian Motion is nowhere (except possibly on set of Lebesgue measure $0$)
  differentiable.}
\solution{}

%%%%%
\problem{ Prove that a Brownian Motion path has no intervals of monotonicity.
...
%%%%%
\problem{ Show that for arbitrarily large $t_1$, there is a $t_2>t_1$ 
  such that $W(t_2)=0$. That is, Brownian Motion paths cross the time-axis for 
  arbitrarily large values of $t$.}
\solution{}

%%%%%
\problem{ Using the fact that the inversion $t W(1/t)$ is also a standard 
...
\problem{ Половина доказательства изометрии \\
Пусть $X(t)$ - адаптированный (согласованный) случайный процесс. \\
Пусть $t_{1}<t_{2}<...<t_{n}$ - фиксированные моменты времени. \\
}
\solution{$$E\left[\left(\sum_{i=1}^{n}X(t_{i})(W(t_{i+1})-W(t_{i}))\right)^{2}\right]=\sum_{i=1}^{n}\E(X^{2}(t_{i}))(t_{i+1}-t_{i})$$}

\problem{
For standard Brownian Motion $W(t)$ find the probability $p \{ W(1)>0 \& W(2)<0 \}$ 
}
\solution{Straight calculation with densities gives
$$
...
\solution{Для <<положительного>> игрока: \\
...
б) может быть явно не решается, а может и решается...\\
Source: plus.math.org/issue15/features/doubling }
...
\problem{For $0<s<t$ calculate the probability $\P(W_t>0 \mid W_s>0)$ and $\P(W_s>0 \mid W_t>0)$. }
\solution{Перейдем к двум независимым нормальным переменным}

\problem{Цель этого упраженения~--- разобраться, сколько времени процесс $Y_t=at+W_t$ в среднем проводит ниже нуля. Другими словами мы хотим найти $\E(T)$, где 
\begin{equation}
...
\end{enumerate}}
\solution{Для нахождения интеграла можно изменить порядок интегрирования. $\E(T)=\frac{\sigma^2}{2\mu^2}$}


\problem{Цель этого упраженения~--- разобраться, сколько времени процесс $Y_t=at+W_t$ в среднем проводит в полоске $[0;y]$. Другими словами мы хотим найти $\E(T)$, где 
\begin{equation}
...
\item Найдите $\E(T)$, если $Y_t=\mu t+\sigma W_t$
\end{enumerate}}
\solution{Для нахождения интеграла можно изменить порядок интегрирования. $\E(T)=y/\mu$}



\section{12. Интеграл Ито}
...
\problem{ Для следующих случайных процессов найдите $dZ_{t}$ и выпишите соответствующую формулу в полной записи (с интегралами вместо $d$): \\
...
\problem{ $dX_{t}=\alpha X_{t}dt+\sigma X_{t}dW_{t}$, $dY_{t}=\gamma Y_{t}dt+\delta Y_{t}dW_{t}$. $Z_{t}=X_{t}/Y_{t}$. Найдите $dZ_{t}$ }
\solution{}

\problem{ $dX_{t}=\alpha X_{t}dt+\sigma X_{t}dW_{1t}$, $dY_{t}=\gamma Y_{t}dt+\delta Y_{t}dW_{2t}$. $W_{1t}$ и $W_{2t}$ - два независимых винеровских процесса. $Z_{t}=X_{t}Y_{t}$. Найдите $dZ_{t}$ }
\solution{}

\problem{ Докажите, что следующие процессы являются мартингалами с использованием и без использования леммы Ито: \\
а) $X_{t}=W_{t}^{2}-t$ \\
...
\solution{ $d(f(t)W_t)=f(t)dW_t+f'(t)W_tdt$, следовательно $\int_0^t f(s)dW_s=f(t)W_t-\int_0^t f'(s)W_sds$ }
...
\problem{ Определим $X_t=\int_0^t \exp(as)\dx{W_s}$. Найдите $\E(X_t)$, $\E(W_t\cdot X_t)$, $\E(X_t\cdot X_s)$ }
\solution{ $0$, Используя изометрию Ито:
\begin{equation}
\E(W_t\cdot X_t)=\E\left(\int_0^t\dx{W_s}\int_0^t\exp(as)\dx{W_s}\right)=\int_0^t 1\cdot \exp(as)\dx{s}
\end{equation}}


\problem{Используя лемму Ито и свойства стохастического интеграла найдите 
\begin{enumerate}
...
\end{enumerate} }
\solution{ $\E(W^{2n}_t)=1\cdot 3\cdot 5 \cdots (2n-3)(2n-1)t^n$ }
...
\problem{ Vasicek interest rate model \\
...
\problem{Решите $dX_t=\frac{1}{X_t}dt+X_tdW_t$ с начальным условием  $X_0=x$}
\solution{Сделаем замену $Y_t = \mathrm{e}^{t- 2 W_t} X_t^2$. Применяем лемму Ито:
\begin{equation}
...
\problem{Consider the following stochastic differential equation
\begin{equation}
...
\end{enumerate}}
\solution{$X_t=\sin(t+W_t)$}

\section{14. Модель Блэка-Шоулса}
...
\problem{ уравнение Блэка-Шоулза для акций с дивидендами \\
...
Подсказка 1: При отсутствии арбитража безрисковый портфель должен приносить такую же доходность, как... \\
