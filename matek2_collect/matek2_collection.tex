...
\item The random variable $X$ is exponentially distributed with parameter $\lambda$. The random variable $Y$ is exponentially distributed with parameter $X$. Find $\E(Y|X)$, $\E(Y)$  and $\Var(Y)$.
...
Find $\E(Y\mid X)$, optimal strategy for risk-neutral player.
...
$dX_t=\frac{X_{\infty}-X_t}{\tau}dt+\sigma dW_t$, where $X_{\infty}$ and $\tau$ are non-random constants, $W_t$ is a Wiener process, and let $Y_t=X_t e^{1/\tau}$.
\begin{enumerate}
