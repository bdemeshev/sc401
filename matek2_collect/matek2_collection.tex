...
\begin{enumerate}
\item Give two non-trivial (other  than $ \Omega $ and $ \emptyset $) examples of event $ A $, such that $ A\in \mathcal{F}_{2014} $.
...
\item Simplify where possible: $ \mathcal{F}_{11}\cap \mathcal{F}_{25} $, $ \mathcal{F}_{11}\cup \mathcal{F}_{25} $, $ \mathcal{H}_{11}\cap \mathcal{H}_{25} $, $ \mathcal{H}_{11}\cup \mathcal{H}_{25} $.
\end{enumerate}
...
\item $Q_t-Q_s=\frac1{\sqrt2}(W_t-W_s)+\frac1{\sqrt2}(V_t-V_s)$ is independent on $\mathcal{F}_s$ since $W_t-W_s$ and $V_t-V_s$ are independent on $\mathcal{F}_s$\\
...
\item $Q_t$ is measurable relative to $\mathcal{F}_t$
\end{enumerate}
...
\item
\begin{enumerate}
\item $\E(M_{t+1}|\mathcal{F}_t)=\E(a^{X_{t+1}}|\mathcal{F}_t)=0.5a^{X_t+1}+0.4a^{X_t-1}+0.1a^{X_t}=a^{X_t}(0.5a+0.4a^{-1}+0.1)$\\
...
\]
\[
X_0=e^{-rT}\E_{\tilde P}(X_T|\mathcal{F}_0)=e^{-rT}\tilde \P(S_T>K|\mathcal{F}_0)=e^{-rT}\tilde \P(S_0e^{(r-0.5\sigma^2)T+\sigma \tilde{W_T}}>K|\mathcal{F}_0)
\]
\[
